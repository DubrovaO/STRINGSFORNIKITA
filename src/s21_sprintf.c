...
int main() {
    ...
    float f = 2.3;
    int dd = 3;
    sprintf(buff2, "%-10.10d%10.3f%+10.4d", d, f, dd);
    printf(buff2);
    printf("\n");

    s21_sprintf(buff3, "%-10.10d%10.3f%+10.4d", d, f, dd);
    printf(buff3);

    /* Test3 */
    printf("TEST3\n");
    sprintf(buff4, "%34.10d%.10d%15d%25.10s", 25, 1234, 4567,
            "HELLOMYDEARFRIEND");
    // sprintf(buff, "%+34.10d%d%15d%25s", 25, 1234, 4567, "HELLOMYDEARFRIEND");
    printf(buff4);
    printf("\n");
    s21_sprintf(buff5, "%34.10d%.10d%15d%25.10s", 25, 1234, 4567,
                "HELLOMYDEARFRIEND");
    printf(buff5);
    ...
    printf("TEST5\n");
    int h = 34;
    sprintf(buff8, "%-25.10d%-30.2d%10.12d%11d%*d%*.*d%+-25.*d%+10.2ld%-11.*hd",
            253, 335, 1234, 34, 10, 25, 5, 15, 345, 5, 666, 456789l, 5, h);
    printf(buff8);
    printf("\n");
    s21_sprintf(buff9,
                "%-25.10d%-30.2d%10.12d%11d%*d%*.*d%+-25.*d%+10.2ld%-11.*hd",
                253, 335, 1234, 34, 10, 25, 5, 15, 345, 5, 666, 456789l, 5, h);
    printf(buff9);
    ...
    char c1 = 'c';
    char c2 = 'T';
    sprintf(buff12, "%10c%-10c%25c%-30c%25c%*c%2c%*c%lc%-50lc", 'a', 'e', 'f',
            'R', 'Q', 10, 'P', 'L', 20, 'x', c1, c2);
    printf(buff12);
    printf("\n");
    s21_sprintf(buff13, "%10c%-10c%25c%-30c%25c%*c%2c%*c%lc%-50lc", 'a', 'e',
    ...
    sprintf(buff13,
            "%-10.f%+-10.f%15.f%+*.10f%*.16f%-10.*f%25.*f%25.1f%+10.2f%*.*f",
    ...
    sprintf(buff15, "%u%15u%-20u%010u%-15.10u%20.*u%*.*u%*.10u%-20.12lu%19hu",
            4321, 34567, 2342, 12346, 234542, 10, 234523, 10, 5, 2345, 20, 2432,
            k, x);
    printf(buff15);
    printf("\n");
    s21_sprintf(
        buff16, "%u%15u%-20u%010u%-15.10u%20.*u%*.*u%*.10u%-20.12lu%19hu", 4321,
        34567, 2342, 12346, 234542, 10, 234523, 10, 5, 2345, 20, 2432, k, x);
    printf(buff16);
    ...
    sprintf(buff17, "%s%23s%-15s%10.s%15.s%16.*s%*.*s%*s%15ls%15.3ls", "aboba",
            "helloworld", "ABOBA", "ABOBBAAA", "AAABOBA", 10, "QWERTY", 15, 4,
            "AAAAABOOOOOBAAA", 10, "ABOBAAAAA", s, s);
    printf(buff17);
    printf("\n");
